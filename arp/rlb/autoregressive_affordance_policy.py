...
class MultiViewTransformer(nn.Module):
    def __init__(self, cfg: DictConfig, renderer: Optional[CubePointCloudRenderer]=None):
        ...
        # 如果需要，可以调整ViT模型的输出维度以匹配你的需求
        if hasattr(self.vit_model, 'classifier'):
            self.vit_model.classifier = nn.Identity()  # 移除分类头
    ...
    def forward(self, img, proprio=None, lang_emb=None):
        """
        :param img: tensor of shape (bs, num_cameras, img_feat_dim, h, w)
        :param proprio: tensor of shape (bs, priprio_dim) [可选]
        :param lang_emb: tensor of shape (bs, lang_len, lang_dim) [可选]
        """
        ...
        for i in range(num_cameras):
            img_i = img[:, i]  # 获取第i个相机的图像
            img_i_transformed = torch.stack([self.vit_transform(image) for image in img_i])  # 应用转换
            with torch.no_grad():  # 如果不需要梯度，可以加速推理并节省内存
                img_i_features = self.vit_model(img_i_transformed).last_hidden_state  # 提取特征
            img_processed.append(img_i_features)

        ins = torch.stack(img_processed, dim=1)  # [B, num_cameras, seq_len, feature_dim]

        # 将ViT提取的特征视为patch特征
        ins = rearrange(ins, "b c l d -> b (c l) d")  # 展平为序列 [B, num_cameras * seq_len, feature_dim]

        # 添加Proprioception特征（如果启用）
        if self.add_proprio:
            p = self.proprio_preprocess(proprio)  # [B,4] -> [B,64]
            p = p.unsqueeze(1).repeat(1, ins.shape[1], 1)  # 扩展到与ins相同的序列长度
            ins = torch.cat([ins, p], dim=-1)  # [B, num_cameras * seq_len, feature_dim + 64]

        # 调整通道顺序和展平（如果需要）

        # 添加位置编码
        if self.pe_fix:
            ins += self.pos_encoding[:, :ins.size(1), :]
        else:
        ...
        if self.add_lang:
            ...
            ins = torch.cat((l, ins), dim=1)  # [B, lang_len + num_cameras * seq_len, feature_dim]

        # 特征变换
        x = self.fc_bef_attn(ins)
        lx, imgx = x[:, :self.lang_max_seq_len], x[:, self.lang_max_seq_len:]
